...
const alcoholStationConfigs: AlcoholStationConfig[] = [
  {
    key: 'bahan-baku',
    nama_stasiun: 'Alkohol - Bahan Baku',
    label: 'Bahan Baku',
    description: 'Analisa bahan baku tetes sebelum proses produksi alkohol.',
  },
  {
    key: 'masakan',
    nama_stasiun: 'Alkohol - Stasiun Masakan',
    label: 'Stasiun Masakan',
    description: 'Parameter brix dan pH adonan pada tahap masakan.',
  },
  {
    key: 'peragian',
    nama_stasiun: 'Alkohol - Stasiun Peragian',
    label: 'Stasiun Peragian',
    description: 'Monitoring beslag, suhu, dan kadar alkohol pada tahap peragian.',
  },
  {
    key: 'sulingan',
    nama_stasiun: 'Alkohol - Stasiun Sulingan / Produksi',
    label: 'Stasiun Sulingan / Produksi',
    description: 'Analisa kadar alkohol hasil sulingan dan produksi.',
  },
  {
    key: 'gudang',
    nama_stasiun: 'Alkohol - Gudang Penimbunan Sementara',
    label: 'Gudang Penimbunan Sementara',
    description: 'Kontrol mutu alkohol nachlop, prima, teknis, dan uji permanganat.',
  },
]
...
const orderedStationList = computed(() => {
  return alcoholStationConfigs
    .map((config) => {
      const station = stasiunList.value.find((item) => item.nama_stasiun === config.nama_stasiun)
      return station ? { ...station, key: config.key, label: config.label, description: config.description } : null
    })
    .filter((item): item is Stasiun & AlcoholStationConfig => item !== null)
})

const currentStation = computed(() => {
  return orderedStationList.value.find((item) => item.id === activeStasiun.value) ?? orderedStationList.value[0] ?? null
})
...
const orderedParameterList = computed(() => [...parameterList.value].sort((a, b) => a.id - b.id))
...
const isHeaderValid = computed(() => !!(activeStasiun.value && form.tanggal && form.jam && isJam24Valid.value && form.shift && form.petugas.trim()))
...
function getTargetText(parameter: Parameter): string {
  ...
  if (parameter.operator_kondisi === 'BETWEEN') {
    return `Target: ${bottom ?? '-'} - ${top ?? '-'}`
  }

  if (parameter.operator_kondisi === 'NONE') {
    return 'Target: Tidak ada batas'
  }

  if (parameter.operator_kondisi === '>' || parameter.operator_kondisi === '>=') {
    return `Target: ${parameter.operator_kondisi} ${bottom ?? '-'}`
  }

  return `Target: ${parameter.operator_kondisi} ${top ?? '-'}`
}

function getSatuan(parameter: Parameter): string {
  return parameter.satuan ?? ''
}
...
async function fetchStasiun() {
  ...
  try {
    ...
    stasiunList.value = data.data ?? []

    if (!activeStasiun.value && orderedStationList.value.length > 0) {
      activeStasiun.value = orderedStationList.value[0].id
    }

    if (orderedStationList.value.length === 0) {
      toast.error('Master stasiun QA alkohol belum tersedia.')
    }

    await nextTick()
    updateTabScrollState()
  } catch (error: any) {
    toast.error(resolveRequestErrorMessage(error, 'Gagal memuat data stasiun QA alkohol.'))
  } finally {
...
async function fetchParametersByStasiun(stasiunId: number) {
  ...
  try {
    ...
    toast.error(resolveRequestErrorMessage(error, 'Gagal memuat parameter QA alkohol.'))
    parameterList.value = []
    form.parameters = []
  } finally {
...
async function fetchHistory(stasiunId: number, page = 1, append = false) {
  ...
  try {
    ...
    toast.error(resolveRequestErrorMessage(error, 'Gagal memuat riwayat QA alkohol.'))
  } finally {
...
<template>
  <div class="space-y-5">
    <div>
      <h1 class="text-xl font-bold text-gray-800 dark:text-white">Entri Data QA Pabrik Alkohol</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
        Parameter produksi alkohol: bahan baku, masakan, peragian, sulingan, dan gudang penimbunan sementara.
      </p>
    </div>
    ...
    <div class="sticky -top-2 z-30 mb-6 bg-white/95 dark:bg-gray-900/90 backdrop-blur-sm rounded-xl border border-gray-200/70 dark:border-gray-800/70 shadow-md">
      <div class="px-3 sm:px-4 lg:px-6 py-0">
        <div v-if="loadingStasiun" class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
          Memuat daftar stasiun QA alkohol...
        </div>
        ...
        <div v-else class="relative">
          ...
          >
            <button
              v-for="stasiun in orderedStationList"
              :key="stasiun.id"
              ...
              <div class="rounded-xl border border-gray-200/70 bg-gray-50/60 dark:border-gray-700/70 dark:bg-gray-900/40 p-3.5">
                <div class="flex items-center justify-between mb-2 gap-2">
                  ...
                  <span
                    ...
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 shrink-0"
                  >
                    <AlertTriangleIcon class="w-3.5 h-3.5" /> Alert
                  </span>
                  <span
                    v-else-if="getStatus(parameter.id) === 'safe'"
                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-900/30 shrink-0"
                  >
    ...
    <div v-else class="min-h-[60vh] flex items-center justify-center">
      <div class="text-center">
        ...
        <p class="text-lg font-semibold text-gray-800 dark:text-white">Master QA alkohol belum siap</p>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Pastikan master stasiun dan parameter QA alkohol sudah tersedia.
        </p>
      </div>
    </div>
  </div>
