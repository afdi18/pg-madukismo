...
const canExportData = computed(() =>
  authStore.canAny(['penerimaan.pemasukan.digiling_pos.export', 'penerimaan.pemasukan.export'])
)
const canPrintData = computed(() =>
  authStore.canAny([
    'penerimaan.pemasukan.digiling_pos.print',
    'penerimaan.pemasukan.print',
    'penerimaan.print',
  ])
)
const canView = computed(() => authStore.can('penerimaan.pemasukan.digiling_pos.view'))
...
function formatCell(column: string, value: unknown): string {
  ...
  if (typeof value === 'string' && key !== 'pos') {
    const num = Number(value)
    if (!isNaN(num)) {
      return num.toLocaleString('id-ID', {
        minimumFractionDigits: 0,
...
async function loadData(background = false) {
  ...
  try {
    const { data } = await axios.get('/api/penerimaan/digiling-per-pos', {
    ...
    console.error(error)
    errorMessage.value = error?.response?.data?.error || error?.message || 'Gagal memuat data digiling per Pos'
    if (!background) {
      rows.value = []
    }
  } finally {
...
function exportToExcel() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Digiling per Pos')
  const filename = `Digiling_Per_Pos_HR${String(hrGil.value).padStart(3, '0')}.xlsx`
        ...
        <table class="w-full min-w-[900px] text-sm">
          ...
          <tbody>
            ...
            <tr v-else-if="rows.length === 0">
              <td :colspan="Math.max(tableColumns.length, 1)" class="px-3 py-6 text-center text-slate-500 dark:text-slate-300">
                Belum ada data digiling per Pos.
              </td>
            </tr>
