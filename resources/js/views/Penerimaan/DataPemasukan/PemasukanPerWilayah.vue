...
const canPrintData = computed(() =>
    authStore.canAny([
        'penerimaan.pemasukan.wilayah.print',
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
        <div class="p-4 space-y-4">
            <div class="flex flex-wrap items-end gap-3 print-hidden">
                ...
                <button @click="onApplyFilter" :disabled="loadingByFilter"
                ...
                <table class="w-full min-w-[820px] text-sm">
                    ...
                    <tbody>
                        <tr v-if="initialLoading">
                            <td colspan="7" class="px-3 py-6 text-center text-slate-500 dark:text-slate-300">Memuat data
                                pemasukan per wilayah...</td>
                        </tr>
                        <tr v-else-if="errorMessage && rows.length === 0">
                            <td colspan="7" class="px-3 py-6 text-center text-red-600 dark:text-red-400">{{ errorMessage
                                }}</td>
                        </tr>
                        <tr v-else-if="rows.length === 0">
                            <td colspan="7" class="px-3 py-6 text-center text-slate-500 dark:text-slate-300">Belum ada
                                data pemasukan per wilayah.</td>
                        </tr>

                        <tr v-for="(row, index) in rows" :key="`${row.nmktgr}-${row.kab}-${index}`"
                            class="bg-cyan-50/70 dark:bg-cyan-950/20 text-gray-800 dark:text-gray-100">
                            <td class="px-2 py-1.5 border border-cyan-200 dark:border-cyan-900/40 text-center">{{ index
                                + 1 }}</td>
                            ...
                            <td class="px-2 py-1.5 border border-cyan-200 dark:border-cyan-900/40 text-right">{{
                                formatNumber(row.rithi) }}</td>
                            <td class="px-2 py-1.5 border border-cyan-200 dark:border-cyan-900/40 text-right">{{
                                formatNumber(row.berathi) }}</td>
                            <td class="px-2 py-1.5 border border-cyan-200 dark:border-cyan-900/40 text-right">{{
                                formatNumber(row.ritsd) }}</td>
                            <td class="px-2 py-1.5 border border-cyan-200 dark:border-cyan-900/40 text-right">{{
                                formatNumber(row.beratsd) }}</td>
                        </tr>

                        <tr v-if="rows.length > 0"
                            class="bg-gray-200/90 dark:bg-gray-800 text-gray-900 dark:text-white font-semibold">
                            <td colspan="3" class="px-2 py-2 border border-gray-300 dark:border-gray-700 text-center">
                                Jumlah</td>
                            <td class="px-2 py-2 border border-gray-300 dark:border-gray-700 text-right">{{
                                formatNumber(totalRitHi) }}</td>
                            <td class="px-2 py-2 border border-gray-300 dark:border-gray-700 text-right">{{
                                formatNumber(totalBeratHi) }}</td>
                            <td class="px-2 py-2 border border-gray-300 dark:border-gray-700 text-right">{{
                                formatNumber(totalRitSd) }}</td>
                            <td class="px-2 py-2 border border-gray-300 dark:border-gray-700 text-right">{{
                                formatNumber(totalBeratSd) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
