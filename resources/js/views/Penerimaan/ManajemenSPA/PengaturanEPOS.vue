...
const mstPosRows = ref<any[]>([])
const loadingMstPos = ref(false)
const updatingMstPosId = ref<string | null>(null)
...
async function loadMstPos() {
  loadingMstPos.value = true
  try {
    const { data } = await axios.get('/api/epos/mstpos')
    mstPosRows.value = data.data || []
  } catch (e: any) {
    console.error(e)
    toast.error(e?.response?.data?.error || e?.response?.data?.message || 'Gagal memuat master POS pengunci SPTA')
  } finally {
    loadingMstPos.value = false
  }
}

async function toggleMstPosKunci(row: any) {
  if (updatingMstPosId.value !== null) return

  pendingMstPosRow.value = row
  pendingMstPosNextKunci.value = Number(row.KUNCI) === 1 ? 0 : 1
  pendingMstPosMessage.value = Number(row.KUNCI) === 1
    ? `Buka kunci cetak SPTA untuk POS ${row.IDPOS}?`
    : `Kunci cetak SPTA untuk POS ${row.IDPOS}?`
  showMstPosConfirm.value = true
}
...
const showMstPosConfirm = ref(false)
const pendingMstPosRow = ref<any | null>(null)
const pendingMstPosNextKunci = ref<number>(0)
const pendingMstPosMessage = ref('')
...
async function confirmMstPosYes() {
  if (!pendingMstPosRow.value) return

  const row = pendingMstPosRow.value
  const nextKunci = pendingMstPosNextKunci.value
  showMstPosConfirm.value = false
  updatingMstPosId.value = String(row.IDPOS)

  try {
    const { data } = await axios.post('/api/epos/mstpos/kunci', {
      idpos: row.IDPOS,
      kunci: nextKunci,
    })

    if (data?.ok) {
      toast.success(`Status kunci POS ${row.IDPOS} diperbarui ke ${nextKunci}`)
      await loadMstPos()
    } else {
      toast.info('Tidak ada perubahan status kunci POS')
    }
  } catch (e: any) {
    console.error(e)
    toast.error(e?.response?.data?.error || e?.response?.data?.message || 'Gagal memperbarui kunci POS')
  } finally {
    updatingMstPosId.value = null
    pendingMstPosRow.value = null
  }
}

function confirmMstPosNo() {
  showMstPosConfirm.value = false
  pendingMstPosRow.value = null
}
  ...
  <div class="space-y-3">
    ...
    <div class="mt-2 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
      <div class="mb-3 flex items-center justify-between gap-3">
        <h3 class="font-semibold dark:text-slate-200">Perangkat Terdaftar</h3>
      ...
      <div v-else class="overflow-x-auto rounded-lg">
        <table class="w-full text-sm table-auto">
          <thead class="bg-slate-50 dark:bg-slate-800">
            <tr class="text-left text-xs text-slate-500 dark:text-white uppercase">
    ...
    <div class="mt-2 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
      <div class="mb-3 flex items-center justify-between gap-3">
        <h3 class="font-semibold dark:text-slate-200">Master Pos Pengunci SPTA (SIMANTEBU)</h3>
        <span class="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:text-slate-200">
          {{ mstPosRows.length }} POS
        </span>
      </div>
      <div v-if="loadingMstPos" class="text-sm text-slate-500">Memuat master POS...</div>
      <div v-else class="overflow-x-auto rounded-lg">
        <table class="w-full text-sm table-auto">
          <thead class="bg-slate-50 dark:bg-slate-800">
            <tr class="text-left text-xs text-slate-500 dark:text-white uppercase">
              <th class="px-4 py-3">IDPOS</th>
              <th class="px-4 py-3">NMPOS</th>
              <th class="px-4 py-3 text-right">KUNCI</th>
              <th class="px-4 py-3 text-right">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in mstPosRows" :key="`mstpos-${row.IDPOS}`" class="hover:bg-slate-50 dark:hover:bg-slate-800">
              <td class="px-4 py-3 font-medium text-slate-700 dark:text-slate-200">{{ row.IDPOS }}</td>
              <td class="px-4 py-3 text-slate-600 dark:text-slate-300">{{ row.NMPOS }}</td>
              <td class="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{{ Number(row.KUNCI) }}</td>
              <td class="px-4 py-3 text-right">
                <button
                  :disabled="updatingMstPosId === String(row.IDPOS)"
                  @click="toggleMstPosKunci(row)"
                  class="px-3 py-1 rounded text-xs font-semibold text-white disabled:opacity-50"
                  :class="Number(row.KUNCI) === 1 ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700'"
                >
                  {{ updatingMstPosId === String(row.IDPOS) ? 'Proses...' : (Number(row.KUNCI) === 1 ? 'Buka Kunci' : 'Kunci') }}
                </button>
              </td>
            </tr>
            <tr v-if="mstPosRows.length === 0">
              <td colspan="4" class="px-4 py-6 text-center text-slate-500">Belum ada data master POS.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- Confirm Modal MSTPOS -->
    <div v-if="showMstPosConfirm" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/50"></div>
      <div class="relative max-w-md w-full mx-4 bg-white dark:bg-gray-900 rounded-lg shadow-lg overflow-hidden">
        <div class="p-4">
          <h3 class="text-lg font-semibold">Konfirmasi Kunci POS</h3>
          <p class="text-sm text-gray-600 mt-2">{{ pendingMstPosMessage }}</p>
          <div class="mt-4 text-right flex gap-2">
            <button @click="confirmMstPosNo" class="px-3 py-2 bg-gray-100 dark:bg-gray-800 rounded">Batal</button>
            <button @click="confirmMstPosYes" class="px-3 py-2 bg-yellow-500 text-white rounded">Ya, Lanjutkan</button>
          </div>
        </div>
      </div>
    </div>
